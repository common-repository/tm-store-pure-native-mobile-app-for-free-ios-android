...
/**
* Return the current used TMS version
*/
function tms_get_version()
{
	global $WORDPRESS_TM_STORE_VERSION;

	return $WORDPRESS_TM_STORE_VERSION;
}
...
* Borrowed from http://wordpress.org/extend/plugins/oa-social-login/
*/
function tms_get_current_url()
{
	//Extract parts
	$request_uri = (isset ($_SERVER ['REQUEST_URI']) ? $_SERVER ['REQUEST_URI'] : $_SERVER ['PHP_SELF']);
	$request_protocol = (tms_is_https_on () ? 'https' : 'http');
	...
	// HOOKABLE:
	$current_url = apply_filters( 'tms_hook_alter_current_url', $current_url );
...
*/
function tms_display_dev_mode_debugging_area( $keyword = 'tms_' )
{
	global $wpdb, $wp_actions , $wp_filter;
?>
<style>
	.tms-dev-nonselectsql {
		color: #a0a !important;
	}
	.tms-dev-expensivesql {
		color: #f44 !important;
	}
	.tms-dev-optionfunc {
		color: #4a4 !important;
	...
	.tms-dev-usedhook, .tms-dev-usedhook a {
		color: #1468fa;
	}
	.tms-dev-usedtmshook {
		color: #a0a !important;
	}
	.tms-dev-unusedhook, .tms-dev-unusedhook a{
		color: #a3a3a3 !important;
	}
	.tms-dev-hookcallback, .tms-dev-hookcallback a {
		color: #4a4 !important;
	}
	.tms-dev-table {
		width:100%;
	...
	.tms-dev-table td, .tms-dev-table th {
...
?>
							...
							<tr>
								<td valign="top" width="450">
									<?php if( stristr( $caller_name, $keyword ) ): ?>
										<a href="https://github.com/hybridauth/WordPress-Social-Login/search?q=<?php echo $caller_name ; ?>" target="_blank" class="tms-dev-tmsfunc"><?php echo $caller_name; ?></a>
									<?php else: ?>
										<a href="https://developer.wordpress.org/?s=<?php echo $caller_name ; ?>" target="_blank" class="tms-dev-nontmsfunc<?php if( stristr( $caller_name, '_option' ) ) echo "- tms-dev-optionfunc"; ?>"><?php echo $caller_name; ?></a>
									<?php endif; ?>
								...
								<td valign="top" class="<?php if( ! stristr( '#' . $sql, '#select ' ) ) echo 'tms-dev-nonselectsql'; ?>"><?php echo nl2br( $sql ); ?></td>
								<td valign="top" width="50" nowrap class="<?php if( $time > 0.05 ) echo 'tms-dev-expensivesql'; ?>"><?php echo number_format( $time, 4, '.', '' ); ?></td>
							</tr>
						<?php

						$total_tms_queries++;
						$total_tms_queries_time += $time;
					}
				}
			}
		?>
		<tr>
			<td colspan="2">Total SQL Queries by TMS : <?php echo $total_tms_queries; ?></td>
			<td width="50" nowrap><?php echo number_format( $total_tms_queries_time, 4, '.', '' ); ?></td>
		</tr>
	</table>
			...
			if( $wp_actions )
			{
				foreach( $wp_actions as $name => $count )
				{
								...
								foreach( $callbacks as $callback )
								{
									if( isset( $callback['function'] ) && is_string( $callback['function'] ) )
									{
										if( stristr( $callback['function'], $keyword ) || stristr( $name, $keyword ) )
										{
											?>
												<tr>
													<td valign="top" width="270" nowrap class="tms-dev-usedhook">
														<?php
															if( stristr( $name, $keyword ) )
															{
																?>
																	<a class="tms-dev-usedtmshook" href="https://github.com/hybridauth/WordPress-Social-Login/search?q=<?php echo $name ; ?>" target="_blank"><?php echo $name ; ?></a>
																<?php
															}
															else
															{
													...
													<td valign="top" class="tms-dev-hookcallback">
														<?php
					...
					elseif( stristr( $name, $keyword )  )
					{
						?>
							<tr>
								<td valign="top" width="270" nowrap class="tms-dev-unusedhook">
	...
	<table class="tms-dev-table">
		<tbody>
			<tr><th width="270"><label>Version</label></th><td><?php echo tms_get_version(); ?></td></tr>
			<tr><th><label>Plugin path</label></th><td><?php echo WORDPRESS_TM_STORE_ABS_PATH; ?></td></tr>
			<tr><th><label>Plugin url</label></th><td><?php echo WORDPRESS_TM_STORE_PLUGIN_URL; ?></td></tr>
			<tr><th><label>HA endpoint</label></th><td><?php echo WORDPRESS_TM_STORE_HYBRIDAUTH_ENDPOINT_URL; ?></td></tr>
		</tbody>
	</table>
...
*/
function tms_generate_backtrace()
{
    $e = new Exception();
    $trace = explode( "\n", $e->getTraceAsString() );
    ...
    for ( $i = 0; $i < $length; $i++ )
    {
        $result[] = ( $i + 1 )  . ')' . str_ireplace( array( realpath( WORDPRESS_TM_STORE_ABS_PATH ), realpath( WP_PLUGIN_DIR . '/' ), realpath( ABSPATH . '/' ) ) , '', substr( $trace[$i], strpos( $trace[$i], ' ' ) ) );
    }
