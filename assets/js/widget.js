...
(function($){ 
	$(function(){ 
		$(document).on( 'click', 'a.wp-tm-store-provider', function(){
			popupurl = $( '#tms_popup_base_url' ).val();
...
*/
window.tms_wordpress_tm_login = function( config ){
	...
	if( ! jQuery( '#loginform' ).length )
	{
		if( jQuery( '#registerform' ).length )
		{
			...
			var login_uri = jQuery( '#tms_login_form_uri' ).val();
